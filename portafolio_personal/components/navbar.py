...
def navbar(lang: str) -> rx.Component:
    return rx.hstack(
            rx.hstack(
                rx.text(
                    "<raul",
                    class_name="font-extrabold text-xl md:text-2x text-slate-400 noto-sans-mono"
                ),
                rx.text(
                    "fdeztdo/>",
                    class_name="font-bold text-xl md:text-2x text-slate-200 noto-sans-mono"
                ),
                spacing="0"
            ),
            rx.hstack(
            ...
            class_name="fixed z-10 top-0 left-0 right-0 flex flex-wrap bg-slate-800 py-3 px-4 md:px-10 w-full",
            align="center",
            justify="between",
        )
